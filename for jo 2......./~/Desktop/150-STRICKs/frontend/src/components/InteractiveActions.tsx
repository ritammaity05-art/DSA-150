...
import { Sparkles, Smile, RefreshCw, Layers, CheckCircle2, HelpCircle, X, ListOrdered, FileCode } from 'lucide-react';
...
export default function InteractiveActions({
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl border border-blue-500/20 bg-slate-950/80 shadow-2xl space-y-6">
      ...
      {/* Additional Example Walkthrough with Full Line-by-Line Cards directly in box */}
      {showExtraExample && (
        <div className="p-5 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 text-xs text-indigo-100 leading-relaxed space-y-4 animate-fade-in font-mono shadow-xl">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-indigo-500/20">
            <div className="flex items-center gap-2 font-sans font-bold text-sm text-indigo-300">
              <ListOrdered className="w-5 h-5 text-amber-400" />
              <span>{isBengali ? "📌 অতিরিক্ত উদাহরণ ও লাইন-বাই-লাইন সলিউশন (Additional Example Walkthrough):" : "📌 Additional Example Walkthrough & Line-by-Line Breakdown:"}</span>
            </div>
            <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-bold border border-emerald-500/30 font-sans">
              Step-by-Step Solved ✓
            </span>
          </div>
          ...
          {/* Dedicated Line-by-Line Execution Breakdown Cards inside this exact box */}
          <div className="space-y-2.5 pt-2">
            <div className="text-[11px] font-bold text-amber-400 uppercase tracking-wider font-sans flex items-center gap-1.5">
              <FileCode className="w-4 h-4 text-amber-400" />
              <span>{isBengali ? "ধাপে ধাপে কোড এক্সিকিউশন লাইন breakdown:" : "Line-by-Line Code Execution Breakdown:"}</span>
            </div>

            <div className="p-3 rounded-xl bg-slate-950 border border-white/10 space-y-1 font-mono text-xs">
              <div className="text-cyan-300 font-bold">Line 1: ans[0] = nums[0] ➔ 22</div>
              <div className="text-gray-300 font-sans text-[11px]">
                {isBengali ? "Index 0 এর মান 22 প্রথম অর্ধে ans[0] = 22 এবং দ্বিতীয় অর্ধে ans[0+4] = 22 কপি করা হলো।" : "Step 1: Index 0 value 22 is copied to ans[0] = 22 and ans[4] = 22."}
              </div>
            </div>

            <div className="p-3 rounded-xl bg-slate-950 border border-white/10 space-y-1 font-mono text-xs">
              <div className="text-cyan-300 font-bold">Line 2: ans[1] = nums[1] ➔ 21</div>
              <div className="text-gray-300 font-sans text-[11px]">
                {isBengali ? "Index 1 এর মান 21 প্রথম অর্ধে ans[1] = 21 এবং দ্বিতীয় অর্ধে ans[1+4] = 21 কপি করা হলো।" : "Step 2: Index 1 value 21 is copied to ans[1] = 21 and ans[5] = 21."}
              </div>
            </div>

            <div className="p-3 rounded-xl bg-slate-950 border border-white/10 space-y-1 font-mono text-xs">
              <div className="text-cyan-300 font-bold">Line 3: ans[2] = nums[2] ➔ 20</div>
              <div className="text-gray-300 font-sans text-[11px]">
                {isBengali ? "Index 2 এর মান 20 প্রথম অর্ধে ans[2] = 20 এবং দ্বিতীয় অর্ধে ans[2+4] = 20 কপি করা হলো।" : "Step 3: Index 2 value 20 is copied to ans[2] = 20 and ans[6] = 20."}
              </div>
            </div>

            <div className="p-3 rounded-xl bg-slate-950 border border-white/10 space-y-1 font-mono text-xs">
              <div className="text-cyan-300 font-bold">Line 4: ans[3] = nums[3] ➔ 1</div>
              <div className="text-gray-300 font-sans text-[11px]">
                {isBengali ? "Index 3 এর মান 1 প্রথম অর্ধে ans[3] = 1 এবং দ্বিতীয় অর্ধে ans[3+4] = 1 কপি করা হলো।" : "Step 4: Index 3 value 1 is copied to ans[3] = 1 and ans[7] = 1."}
              </div>
            </div>
          </div>
          ...
          <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-sans font-semibold flex items-center justify-between">
            <span>{isBengali ? "চুড়ান্ত সমাধান সম্পন্ন:" : "Final Verified Output:"}</span>
